...
class BasketComponent extends BasketComponent_parent
{
    ...
    protected function formatUrl($unformedUrl)
    {
        ...
        $url = \OxidEsales\Eshop\Core\Registry::get(\OxidEsales\Eshop\Core\UtilsUrl::class)->processUrl($url);

        $seoIsActive = \OxidEsales\Eshop\Core\Registry::getUtils()->seoIsActive();
        if ($seoIsActive && $seoUrl = \OxidEsales\Eshop\Core\Registry::get(\OxidEsales\Eshop\Core\SeoEncoder::class)->getStaticUrl($url)) {
            $url = $seoUrl;
        }
