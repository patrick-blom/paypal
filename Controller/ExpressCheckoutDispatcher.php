...
class ExpressCheckoutDispatcher extends \OxidEsales\PayPalModule\Controller\Dispatcher
{
    ...
    public function setExpressCheckout()
    {
        ...
        try {
            ...
            if (!$validator->isPaymentValid()) {
                /**
                 * @var \OxidEsales\PayPalModule\Core\Exception\PayPalException $ex
                 */
                $ex = oxNew(\OxidEsales\PayPalModule\Core\Exception\PayPalException::class);
                $ex->setMessage(\OxidEsales\Eshop\Core\Registry::getLang()->translateString("OEPAYPAL_PAYMENT_NOT_VALID"));
                throw $ex;
            }
     ...
     *
     * @param array $deliveryListSet delivery list
     *
     * @return array
     */
    public function makeUniqueNames($deliveryListSet)
    {
        $result = array();
        $nameCounts = array();

        foreach ($deliveryListSet as $deliveryList) {
            $deliveryListName = trim($deliveryList->oxdeliveryset__oxtitle->value);

            if (isset($nameCounts[$deliveryListName])) {
                $nameCounts[$deliveryListName] += 1;
            } else {
                $nameCounts[$deliveryListName] = 1;
            }

            $suffix = ($nameCounts[$deliveryListName] > 1) ? " (" . $nameCounts[$deliveryListName] . ")" : '';
            $result[$delSet->oxdeliveryset__oxid->value] = $deliveryListName . $suffix;
        }
        ...
        if ($userId = $user->isRealPayPalUser($userEmail)) {
            ...
            if (!$loggedUser) {
                if (!$user->isSamePayPalUser($details)) {
                    /**
                     * @var $ex \OxidEsales\Eshop\Core\Exception\StandardException
                     */
                    $ex = oxNew(\OxidEsales\Eshop\Core\Exception\StandardException::class);
                    $ex->setMessage('OEPAYPAL_ERROR_USER_ADDRESS');
                    throw $ex;
                }
